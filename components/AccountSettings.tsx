...
const AccountSettings: React.FC<Props> = ({ user }) => {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
        ...
        Swal.fire({
            title: 'Berhasil',
            text: password ? 'Nama dan Password berhasil diperbarui.' : 'Nama berhasil diperbarui.',
            icon: 'success'
        });
  ...
  return (
    <div className="w-full max-w-2xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      ...
      <form onSubmit={handleUpdate} className="bg-white p-8 rounded-2xl border border-slate-200 shadow-soft space-y-6">
          
          <div className="p-4 bg-slate-50 rounded-xl flex items-center gap-4 mb-4">
          ...
          <div className="border-t border-slate-100 pt-4 mt-4">
              ...
              <InputGroup label="Password Baru (Opsional)" icon={Lock} value={password} onChange={setPassword}>
                  <input 
                      type="password"
                      value={password}
